...
const MyChip = ({ label, color, variant, size, rounded = 'full', dot = false, startAdornment, endAdornment }) => {
    const makeStyle = React.useMemo(() => {
        let className = "w-max h-max flex items-center";

        if (color === "gray" && variant === "filled") {
            className += " text-gray-light/700 border border-gray-light/200 bg-gray-light/50";
            ...
            className += " text-gray-light/700 border border-gray-light/200 bg-white shadow-shadows/shadow-xs";
        } else if (color === "modern" && variant === "outlined") {
            className += " text-gray-light/700 border border-gray-light/300 bg-white shadow-shadows/shadow-xs";
        }
    ...
    return (
        <>
            <div className={makeStyle}>
                {dot && <div className={`w-2 h-2 min-h-[8px] min-w-[8px] rounded-full ${getDotClass}`}> </div>}
