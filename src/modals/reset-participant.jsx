...
import { resetParticipant as resetParticipantSchema } from "../yup/schema";
import { yupResolver } from "@hookform/resolvers/yup";

const Modal = ({ data = {}, done, close }) => {
    const { setValue, handleSubmit, register, control, watch, formState: { errors } } = useForm({ resolver: yupResolver(resetParticipantSchema) });
    ...
    const resetParticipant = async (body) => {
        var input = {
            reset_data_password: body.reset_data_password
        }

        await Reguest.resetParticipant(input)
            .then(myToaster)
            .then(handleDone)
            .catch(myToaster);
    };
    const submit = resetParticipant;
                        ...
                        <header className="px-6 pt-6 pb-5  relative">
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="z-30 flex flex-col items-start justify-start gap-1">
                                    <p className="text-lg-semibold text-gray-light/900">Reset Peserta Undian</p>
                                    <p className="text-sm-regular text-gray-light/600">Perhatian! Melakukan reset berarti akan menghapus seluruh data peserta</p>
                                </div>
                            </div>
                        </header>
